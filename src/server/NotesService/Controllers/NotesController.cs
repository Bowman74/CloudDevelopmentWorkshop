    ...
    public class NotesController : ApiController
    {
        ...
        public async Task<Note> Post(Note note)
        {
            try
            {
                var client = await GetClientAsync();
                await
                    client.CreateDocumentAsync(UriFactory.CreateDocumentCollectionUri(documentDatabaseName, documentCollectionName), note);
        ...
        public async Task<Note> Put(string id, Note note)
        {
            try
            {
                var client = await GetClientAsync();
                await
                    client.ReplaceDocumentAsync(UriFactory.CreateDocumentUri(documentDatabaseName, documentCollectionName, id), note);
            }
            catch (Exception ex)
            {
                Request.CreateResponse(HttpStatusCode.InternalServerError, ex.ToString());
        ...
        public async Task<HttpResponseMessage> Delete(string id)
        {
            ...
            {
                var client = await GetClientAsync();
                await client.DeleteDocumentAsync(UriFactory.CreateDocumentUri(documentDatabaseName, documentCollectionName, id));
